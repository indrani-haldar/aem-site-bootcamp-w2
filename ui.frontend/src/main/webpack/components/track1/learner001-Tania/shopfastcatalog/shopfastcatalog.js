class ShopFastCatalog {
    ...
    bindEvents() {
        document.addEventListener('shopfast:filterschange', (event) => {
        ...
        document.addEventListener('click', (event) => {
            ...
            if (addButton) {this.addToCart(Number(addButton.dataset.addProduct));}
            const cartButton = event.target.closest('[data-cart-action]');
            if (cartButton) {this.updateCart(Number(cartButton.dataset.cartId), cartButton.dataset.cartAction);}
        });
    ...
    renderProducts() {
        ...
        this.grid.innerHTML = visibleProducts.length ? visibleProducts.map((product) => {
            ...
            return `<article class="shopfast-card"><div class="shopfast-card__image-wrap"><img src="${this.escape(image)}" alt="${this.escape(product.title)}" loading="lazy"></div><div class="shopfast-card__body"><h2>${this.escape(product.title)}</h2><strong>$${Number(product.price).toFixed(2)}</strong><button type="button" data-add-product="${product.id}">Add to cart</button></div></article>`;
    ...
    renderCart() {
        ...
        this.cartPanel.querySelector('[data-cart-items]').innerHTML = this.cart.length ? this.cart.map((item) => {
            ...
            return `<div class="shopfast-cart-item"><img src="${this.escape(image)}" alt="${this.escape(item.title)}"><div><strong>${this.escape(item.title)}</strong><span>$${Number(item.price).toFixed(2)}</span><div class="shopfast-cart-item__controls"><button type="button" data-cart-action="decrease" data-cart-id="${item.id}" aria-label="Decrease ${this.escape(item.title)} quantity">-</button><span>${item.quantity}</span><button type="button" data-cart-action="increase" data-cart-id="${item.id}" aria-label="Increase ${this.escape(item.title)} quantity">+</button><button type="button" data-cart-action="remove" data-cart-id="${item.id}" aria-label="Remove ${this.escape(item.title)} from cart">Remove</button></div></div></div>`;
    ...
    openCart() {
        ...
        this.cartPanel.querySelector('.shopfast__cart-close')?.focus();
    }
    ...
    loadCart() {
        try {
            return JSON.parse(sessionStorage.getItem('shopfast-cart-learner001-Tania')) || [];
        } catch (error) {
            return [];
        }
    }
    ...
    saveCart() {
        sessionStorage.setItem('shopfast-cart-learner001-Tania', JSON.stringify(this.cart));
    }
